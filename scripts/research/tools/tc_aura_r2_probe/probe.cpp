// R2 hostile-review probe (aura-lifecycle).  Scaffolding copied from tools/tc_aura_periodic_probe/probe.cpp
// (same cut points, stated there); tc_r2_*.inc are Trinity's text (extract.py).  Extra commands:
//   addaura <id>            Unit::AddAura refresh: ModStackAmount(1, DEFAULT, AuraCreateInfo default ResetPeriodicTimer)
//                           (glue = Unit.cpp:12314 -> SpellAuras.cpp:350 -> Unit.cpp:3441; no Spell.cpp duration commit)
//   modstacks <id> <value>  SPELL_EFFECT_MODIFY_AURA_STACKS MiscValue 0 (SpellEffects.cpp:6138, verbatim)
//   painful <id> <ms>       spell_pri_painful_punishment extension (spell_priest.cpp:2995, verbatim)
//   mentaldecay <id> <ms>   spell_pri_mental_decay extension (spell_priest.cpp:2847-2848, verbatim)
//   state <id>              print the aura
// Differential probe for the periodic-timer oracle (aura_lifecycle/periodic.py, Track D).
//
// tc_r2_decls.inc / tc_r2_bodies.inc are pulled verbatim out of the sibling
...
#include "tc_r2_bodies.inc"

#include "tc_r2_extra.inc"
...
static void CreateAura(Unit& owner, uint32 id)
{
...
int main()
{
    ...
    while (std::cin >> cmd)
    {
        ...
        else if (cmd == "addaura" || cmd == "modstacks" || cmd == "painful" || cmd == "mentaldecay" || cmd == "state")
        {
            uint32 id; std::cin >> id;
            int32 v = 0;
            if (cmd == "modstacks" || cmd == "painful" || cmd == "mentaldecay") std::cin >> v;
            Aura* a = FindAura(owner, id);
            if (!a) { std::cout << "{\"event\":\"" << cmd << "\",\"aura\":null}\n"; continue; }
            if (cmd == "addaura") a->ModStackAmount(1, AURA_REMOVE_BY_DEFAULT, AuraCreateInfoDefaultResetPeriodicTimer);
            else if (cmd == "modstacks") ProbeEffectModifyAuraStacks(a, v);
            else if (cmd == "painful") ProbePainfulPunishment(a, v);
            else if (cmd == "mentaldecay") ProbeMentalDecay(a, v);
            std::ostringstream o; o << "{\"event\":\"" << cmd << "\",\"aura\":"; PrintAura(o, a); o << "}";
            std::cout << o.str() << "\n";
        }
        else if (cmd == "remove")
        {
