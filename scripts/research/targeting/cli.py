"""``targeting.py`` command line.

Commands are contributed by ``targeting/cmd_*.py`` modules.  Each defines::
...
def _commands() -> dict[str, tuple]:
    ...
    for mod in sorted(pkgutil.iter_modules(targeting.__path__), key=lambda m: m.name):
        if not mod.name.startswith("cmd_"):
            continue
        module = importlib.import_module(f"targeting.{mod.name}")
        for name, spec in getattr(module, "COMMANDS", {}).items():
            if name in found:
                raise RuntimeError(f"duplicate targeting command {name!r} ({mod.name})")
            found[name] = spec
    return found
...
def main(argv: list[str] | None = None) -> int:
    commands = _commands()
    parser = argparse.ArgumentParser(prog="targeting.py", description=__doc__.splitlines()[0])
